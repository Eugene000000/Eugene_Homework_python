...
with allure.step("Нажатие на кнопку SUBMIT"):
    def submit(self, submit_button: str) -> None:
        self.driver.find_element(*self._submit_button).click()
        """
        Эта функция для нажимания кнопки "Submit"
        """
with allure.step("Проверка цвета поля ZIP_CODE, оно красное"):
...
with allure.step("Проверка цветов других полей, они зелёные"):
